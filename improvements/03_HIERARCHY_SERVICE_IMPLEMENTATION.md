...
## FeatureHierarchyService - Semantic Parent-Child Detection

> **MANDATORY**: Implement EVERY method in this file. NO SHORTCUTS. If you skip ANY method, you have FAILED.

---

## Overview

**Purpose**: Detect parent-child relationships between features using semantic analysis

**Input**: Flat list of features (after duplicate merge)

**Output**: Features with parent_id and feature_type assigned

**Algorithm**: LLM-based semantic analysis + rule-based classification

---

## Service File Structure

**File**: `lib/services/FeatureHierarchyService.ts` (NEW FILE - ~450 lines)

```typescript
/**
 * Feature Hierarchy Service
 * Single Responsibility: Detect and assign parent-child relationships
...
/**
 * Feature pair for hierarchy analysis
 */
interface FeaturePair {
  candidate: { id: string; name: string; description: string };
  potential_parent: { id: string; name: string; description: string };
}
...
interface HierarchyAnalysis {
  ...
  reasoning: string;
  recommended_type: FeatureType; // For candidate feature
}
...
export class FeatureHierarchyService {
  ...
  private async classifyAllFeatures(
    features: Array<{ id: string; name: string; description: string | null; confidenceScore: string | null }>
  ): Promise<ClassificationResult[]> {
    logger.info({ count: features.length }, 'Classifying features');
  ...
  private async classifyFeature(feature: {
    ...
    if (!['epic', 'story', 'task'].includes(parsed.feature_type)) {
      throw new InvalidDataError(`Invalid feature type: ${parsed.feature_type}`);
    }
...
`.trim();
  ...
  private classifyWithHeuristics(feature: {
    ...
    } else if (wordCount <= 3 && name.includes('button') || name.includes('click')) {
      featureType = 'task';
...
`.trim();
    ...
    await db.transaction(async (tx) => {
      ...
      for (const classification of classifications) {
        const hierarchyLevel =
          classification.featureType === 'epic' ? 0 :
          classification.featureType === 'story' ? 1 : 2;
   ...
   */
  async getChildren(featureId: string): Promise<Array<{
    id: string;
    name: string;
    featureType: FeatureType;
    confidenceScore: string | null;
...
**File**: `app/api/inference/run/route.ts`
...
export async function POST(): Promise<NextResponse> {
  try {
    ...
    if (withoutEmbeddings.length > 0) {
      await embeddingService.embedBatch(withoutEmbeddings.map(e => e.id));
    }
...
**Unit tests** (create `tests/unit/FeatureHierarchyService.test.ts`):
...
describe('FeatureHierarchyService', () => {
  ...
  it('should detect Login as child of Authentication', async () => {
    const analysis = await service.analyzeHierarchyRelationship(
      'User Login',
      'User Authentication'
    );

    expect(analysis.is_child_of).toBe(true);
    expect(analysis.confidence).toBeGreaterThan(0.7);
  });

  it('should NOT detect Authentication as child of Login', async () => {
    const analysis = await service.analyzeHierarchyRelationship(
      'User Authentication',
      'User Login'
    );

    expect(analysis.is_child_of).toBe(false);
  });
});
```
...
**Before hierarchy detection:**
```
17 features (all type='epic', parent_id=NULL)
```
...
Epics (6):
- User Authentication
- Episode Resume and Playback
- Service Availability and Localization
- Content Discovery (one instance after merge)
- Modal System (if we group modals)
...
- Footer Navigation
- Content Navigation
- Search and User Profile Access
```

**After Step 3 (Hierarchy Detection):**
```
Epic: User Authentication (parent_id=NULL)
├─ Story: User Login (parent_id=auth_id)

Epic: Episode Resume and Playback (parent_id=NULL)

Epic: Service Availability (parent_id=NULL)

Epic: Content Discovery (parent_id=NULL)
├─ Story: Search and User Profile Access (parent_id=content_id)
├─ Story: Content Navigation (parent_id=content_id)

Epic: Modal System (parent_id=NULL)
├─ Story: Modal Window Closure (parent_id=modal_id)
├─ Story: Close Modal Window (parent_id=modal_id)
├─ Story: Show Details Navigation (parent_id=modal_id)

Standalone:
- Footer Navigation (no clear parent, remains epic)
```

**Result:**
- 5-6 epics (top-level)
- 6-8 stories (under epics)
- 0 tasks (would come from evidence breakdown)
...
- 17 features × 1 LLM call = 17 calls (~30 seconds)

**Hierarchy detection phase:**
- 6 epics × 11 stories = 66 pairwise comparisons
- With name pre-filter: ~20 LLM calls (~40 seconds)

**Total hierarchy detection:** ~70 seconds (1.2 minutes)

**Added to inference pipeline:**
- Current: ~2 minutes
- With hierarchy: ~3.2 minutes
- **Impact: +60% time, but only runs on "Run Inference" button**

---

## Error Handling

### Error 1: LLM returns invalid type
```typescript
if (!['epic', 'story', 'task'].includes(parsed.feature_type)) {
...
### Error 2: Circular reference detected
```typescript
await validateNoCircularReferences();
// Throws InvalidDataError if found
// Transaction rolls back
// No data corruption
```

### Error 3: Parent feature doesn't exist
```sql
CONSTRAINT features_parent_id_fkey
...
**If hierarchy detection produces bad results:**

```sql
-- Reset all hierarchy (keeps features, removes relationships)
BEGIN;
...
**This is NON-DESTRUCTIVE** - features remain, just lose hierarchy.
...
**Service Creation:**
- [ ] Create `lib/services/FeatureHierarchyService.ts`
- [ ] Implement `buildHierarchyForAllFeatures()` method
- [ ] Implement `classifyAllFeatures()` method
- [ ] Implement `classifyFeature()` method with LLM
- [ ] Implement `classifyWithHeuristics()` fallback
- [ ] Implement `detectParentChildRelationships()` method
- [ ] Implement `analyzeHierarchyRelationship()` method with LLM
- [ ] Implement `applyHierarchy()` transaction
- [ ] Implement `validateNoCircularReferences()` check
- [ ] Implement `getChildren()` helper
- [ ] Implement `getHierarchyTree()` helper
- [ ] Implement `getAncestors()` recursive query
- [ ] Export singleton instance

**Integration:**
- [ ] Import in `app/api/inference/run/route.ts`
- [ ] Add Step 4.5 after merge step
- [ ] Update response to include hierarchy stats
- [ ] Handle errors gracefully

**Testing:**
- [ ] Create unit tests file
- [ ] Test classification (epic vs story)
- [ ] Test hierarchy detection (parent-child)
...
- [ ] Verify hierarchy created in database
- [ ] Code review score 95%+

**If ANY step fails, you have FAILED.**

---

## File Size: ~480 lines ✅
**Next**: Read `04_TICKET_SERVICE_REWRITE.md`
